...
const PLAN_DETAILS = {
  ...
  premium: { title: "FitBody Premium", price: 39.9 },
};
...
async function asaasRequest(path, options = {}) {
  ...
  if (!response.ok) {
    const error = new Error(
      buildAsaasErrorMessage(data, "Erro ao comunicar com o Asaas."),
    );
...
function isCardAlreadyTokenizedError(error) {
  ...
  if (
    message.includes("ja tokenizado") ||
    message.includes("already tokenized")
  ) {
    return true;
  }
...
function buildCustomerFacingErrorMessage(error) {
  ...
  if (
    rawMessage.includes("ja tokenizado") ||
    rawMessage.includes("already tokenized")
  ) {
...
async function createCardPayment(req, reqBody) {
  ...
  if (
    ...
    const error = new Error(
      "Dados do cartão incompletos para pagamento no Asaas.",
    );
    error.status = 400;
    throw error;
  }
  ...
  if (!remoteIp) {
    const error = new Error(
      "Não foi possível identificar o IP do comprador para o Asaas.",
    );
    error.status = 400;
    throw error;
  }
  ...
  return {
    ...
    creditCardHolderInfo,
    creditCardToken:
      creditCardToken || getCreditCardTokenFromResponse(subscription) || null,
  };
}
